...
def override_list(source, target):
...
def simulate(_map_with_meta, after):
    ...
    y_range = range(0, _map_high)
    x_range = range(0, _map_width)
    with open('stats.txt', mode='wt', encoding='utf-8') as fd:
        for minute in range(0, after + 1):
            override_list(_map, minute_map_snapshot)
            for y in y_range:
                for x in x_range:
                    wood_acres, lumberyard_acres, empty_acres = gather_statistics_for_field(
                        minute_map_with_meta_snapshot,
                        x, y)
            ...
            fd.write('{},{},{}\n'.format(wood_acres, lumberyard_acres, empty_acres))
            if minute % 100 == 0:
                print(minute)
...
if __name__ == '__main__':
    ...
    simulate(map_with_meta, 1000000000)
    print_map(map_with_meta)
    wood_acres, lumberyard_acres, empty_acres = compute_map_statistics(map_with_meta)
    print(wood_acres * lumberyard_acres)


# In file I found regularity 
# 6780
# 6808 | 28
# 6836 | 28
# 6864 | 28
